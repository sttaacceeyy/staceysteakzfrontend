...
const defaultSales = [
  { date: '2025-06-24', branch: 'Downtown', total: 500 },
  { date: '2025-06-25', branch: 'Uptown', total: 650 },
];
function getSales() {
  const data = localStorage.getItem(SALES_KEY);
  return data ? JSON.parse(data) : defaultSales;
}
const HQManagerSales: React.FC = () => {
  ...
  return (
    <div className="container mx-auto p-4 max-w-4xl">
      <h1 className="text-2xl font-bold mb-4">All Branch Sales Analytics</h1>
      <table className="w-full text-left border mb-2">
        ...
        <tbody>
          {sales.map((s: any) => (
            <tr key={s.date + s.branch} className="border-t">
